...
class DataCenterMonitor:
    ...
    def estimate_power(metrics):
        ...
        try:
            ...
            st.error(f"Power estimation error: {str(e)}")
            return None
    ...
    **Addressing cloud infrastructure challenges:**  
    ...
    with col1:
        ...
        if len(st.session_state.metrics_history) > 10:
            ...
            last_power = st.session_state.metrics_history['total_power'].iloc[-1] if not st.session_state.metrics_history.empty else 0
    ...
    if metrics:
        ...
        emissions = monitor.calculate_emissions(power["total_power"] if power else None)
        
        if power and emissions:
